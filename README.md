...
물리 교사를 위한 AI 기반 수준별 맞춤 문제지 자동 생성 도구입니다. 학급 성적 데이터(Excel/CSV)를 입력받아 Claude AI가 학생 수준을 분석하고, 수준별 맞춤 문제를 생성한 뒤 인쇄 가능한 PDF로 출력합니다.
...
1. **① API 설정**: Anthropic API 키 입력 및 연결 테스트
...
2. 회원가입 또는 로그인합니다
3. **API Keys** 메뉴에서 **Create Key**를 클릭합니다
4. 생성된 키를 복사하여 프로그램의 ① API 설정 탭에 입력합니다

> API 키는 `sk-ant-` 로 시작합니다. 키는 로컬에만 저장되며 외부로 전송되지 않습니다.
...
- API 키가 `sk-ant-`로 시작하는지 확인하세요.
- 인터넷 연결을 확인하세요.
- Anthropic Console에서 API 사용량 한도를 확인하세요.
...
- 수준별 10문항 기준으로 각 수준당 약 30초~1분 정도 소요됩니다.
- 3개 수준이 병렬로 처리되므로 전체 약 1분 내외입니다.
